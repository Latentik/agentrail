...
from agentrail.prompt.claude import render_claude_prompt
...
class ClaudeAdapter:
    ...
    def extract_excerpt(
        ...
        return extract_file_excerpt(self.name, "claude-transcript.excerpt.md", discovery)

    def render_prompt(self, context: HandoffContext) -> PromptArtifact:
        return PromptArtifact(
            target=self.name,
            filename="next-prompt.claude.md",
            content=render_claude_prompt(context),
        )
